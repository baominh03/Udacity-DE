...
def process_song_data(spark, input_data, output_data):
    ...
    artists_table = df.select("artist_id", "artist_name", col("artist_location").alias("location"), col(
        "artist_latitude").alias("latitude"), col("artist_longitude").alias("longitude")).dropDuplicates()
...
def process_log_data(spark, input_data, output_data):
    ...
    users_table = df.select(col("userId").alias("user_id"), col("firstName").alias(
        "first_name"), col("lastName").alias("last_name"), "gender", "level").dropDuplicates()

    # write users table to parquet files
    users_table.write.mode('overwrite').parquet(output_data + 'users_table/')

    # create datetime column from original timestamp column
    # refer: Lesson 2 - from Data Wrangling with Spark Exercise '4_data_wrangling' - Calculating Statistics by Hour
    ...
    time_table = df.withColumn("hour", hour("start_time"))\
        ...
        .select("ts", "start_time", "hour", "day", "week", "month", "year", "weekday").dropDuplicates()   # refer: https://knowledge.udacity.com/questions/806719
    ...
    songplays_table = df.join(song_df, song_df.title == df.song, how='inner') \
    ...
    songplays_table.drop_duplicates().write.mode('overwrite').partitionBy(
